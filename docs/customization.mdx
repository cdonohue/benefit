...
} from "../dist/benefit.js"

By importing and using the `ConfigProvider` from `benefit`, you have the ability to modify the existing configuration or start entirely from scratch using your own design system.
...
```js
{
  theme, reset, utilities, variants, apply
}
```
...
The `reset` key holds any rules used for applying any base css prior to adding any parsed utility classes. These keys should be named after the element they represent (like `a` or `section`), except for a special `base` key that will be applied to all elements. Provide a function as the value of the `reset` key and have access to any `theme` values you define above.

Using the default `base` reset as an example here to showcase using the defined theme values to help you apply any reset styles:
...
If you wanted to add a single utility for branding, like `bg-primary`, you would do the following:
...
<Box className="rounded bg-white shadow">
  <Box is="code" className="font-mono p-4 block border-b">
    ...
    <Box className="inline text-purple-600">utilities</Box>, <Box className="inline text-orange-600">
      generateDeclarations
    </Box>, <Box className="inline text-green-600">prefix</Box>)
  </Box>
  <Box className="p-4">
    <Box className="bg-purple-100 rounded p-2 mb-2 border border-purple-200">
      <Box className="inline text-purple-600 font-mono">utilities</Box> defines
